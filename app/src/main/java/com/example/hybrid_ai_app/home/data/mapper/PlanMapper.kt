...
fun WorkoutPlanDto.toDomain(): ActivePlan {
    ...
    val parsedDate = try { format.parse(this.startDate) ?: Date() } catch (e: Exception) { Date() }
...
fun WeekDto.toDomain(): PlanWeek = PlanWeek(weekNumber = this.weekNumber, days = this.days.map { it.toDomain() })

fun DayDto.toDomain(): PlanDay = PlanDay(dayName = this.dayName, exercises = this.exercises.map { it.toDomain() })

fun ExerciseDto.toDomain(): Exercise = Exercise(name = this.name, sets = this.sets, reps = this.reps, rpe = this.rpe)